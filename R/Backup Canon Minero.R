...
getwd()
setwd("C:/Users/esteb/OneDrive/Documents/Esteban/Berlin/General/MA LA Studien/MA")
...
short_years <- sprintf("%02d", 4:23)     # "04" to "24"
full_years <- 2004:2023                 # 2004 to 2024
...
df_remove <- paste0("Transferencias_Regionales_", sprintf("%02d", 4:23))
rm(list = df_remove)
...
##Limpiar filas
for (y in sprintf("%02d", 4:23)) assign(paste0("Transferencias_Municipales_", y),
...
##Remove Transferencias_Regionales_XX
df_remove <- paste0("Transferencias_Municipales_", sprintf("%02d", 4:23))
rm(list = df_remove)
...
Transferencias_Provinciales <- Transferencias_Municipales %>%
  filter(grepl("MUN\\. PRO\\.|^MUNICIPALIDAD PROVINCIAL |MUNICIPALIDAD METROPOLITANA DE", name)) %>% 
  mutate(
...
Transferencias_Municipales <- Transferencias_Municipales %>% 
  mutate(name = str_remove(name, "^MUN\\. DIS\\. DE "),
...
Mapa_Departamento <- map_DEP #Lima Provincias y Metropolitana en una sola
...
Mapa_Departamento <- st_as_sf(Mapa_Departamento)
Departamentos_Mineros <- unique(Transferencias_Regionales$name)
...
Mapa_Departamento <- Mapa_Departamento %>%
  mutate(DEPARTAMENTO_UPPER = toupper(DEPARTAMENTO))

Mapa_Departamento <- Mapa_Departamento %>%
  mutate(
    DEPARTAMENTO_UPPER = stri_trans_general(DEPARTAMENTO_UPPER, "Latin-ASCII"),
    Mineria = ifelse(DEPARTAMENTO_UPPER %in% Departamentos_Mineros, "Mining Region", "Non-mining Region")
  )

ggplot(Mapa_Departamento) +
  geom_sf(aes(fill = Mineria), color = "white") +
  geom_sf(data = Mapa_Departamento, fill = NA, color = "black", size = 0.3) +
  scale_fill_manual(values = c("Mining Region" = "goldenrod", "Non-mining Region" = "gray80")) +
  geom_text_repel(data = Mapa_Departamento,
                  aes(x = coords_x, y = coords_y, label = DEPARTAMENTO_UPPER),
                  size = 2.5, fontface = "bold",
                  max.overlaps = 20, box.padding = 0.4, point.padding = 0.2, segment.color = "gray60") +
  labs(title = "Mining vs. Non-Mining Regions in Peru", fill = "") +
  theme_minimal()

setdiff(Mapa_Departamento$DEPARTAMENTO_UPPER, Departamentos_Mineros) #No existen departamentos que no reciban Canon!
...
Mapa_Provincia <- Mapa_Provincia %>%
  mutate(PROVINCIA_UPPER = toupper(PROVINCIA),
         PROVINCIA_UPPER = case_when(
           ...
           PROVINCIA_UPPER == "LIMA" ~ "LIMA METROPOLITANA",
...
Transferencias_Provinciales <- Transferencias_Provinciales %>% 
  mutate(name = case_when(
    ...
    TRUE ~ name
  ))  #Qué pasa con CASCAS, HUACHO, SAN JOSE DE SISA
#Al parecer son distritos municipales -> aparecen en Mapa_Distrito
#Revisar LIMA en Transferencias_Provinciales, a qué se refiere? Genera
#discrepancia con Mapa_Provincia

Provincias_Mineras <- unique(Transferencias_Provinciales$name)

Mapa_Provincia <- Mapa_Provincia %>%
  mutate(
    Mineria = ifelse(PROVINCIA_UPPER %in% Provincias_Mineras, "Mining Province", "Non-mining Province")
  )

ggplot(Mapa_Provincia) +
  geom_sf(aes(fill = Mineria), color = "white") +
  geom_sf(data = Mapa_Provincia, fill = NA, color = "black", size = 0.3) +
  scale_fill_manual(values = c("Mining Province" = "goldenrod", "Non-mining Province" = "gray80")) +
  labs(title = "Mining vs. Non-Mining Provinces in Peru", fill = "") +
  theme_minimal()
...
Mapa_Distrito <- Mapa_Distrito %>%
  mutate(
    Mineria = ifelse(DISTRITO_UPPER %in% Distritos_Mineros, "Mining District", "Non-mining District"))

ggplot(Mapa_Distrito) +
  geom_sf(aes(fill = Mineria), color = "white") +
  geom_sf(data = Mapa_Distrito, fill = NA, color = "black", size = 0.3) +
...
#Duplicados <- Mapa_Distrito %>%
#filter(duplicated(DISTRITO_UPPER) | duplicated(DISTRITO_UPPER, fromLast = TRUE))
#All except for BARRANCA are Mining Districts and have the same label.

length(setdiff(Mapa_Distrito$DISTRITO_UPPER, Distritos_Mineros)) #Existen 65 distritos no mineros
...
#el grupo de comparación aumenta a 444 observaciones.
...
###Load Data###
#Ubigeo_provincia <- read.csv("./Data/Ubigeo/ubigeo_provincia.csv")
#Ubigeo_distrito <- read.csv("./Data/Ubigeo/ubigeo_distrito.csv")
...
for(i in seq_along(full_years)) {
  ...
  work_path = paste0(ENAHO_path, full_years[i])
  print(full_years[i])
  
  folders_only = list.files(work_path, full.names = TRUE)
  folders_only = folders_only[file.info(folders_only)$isdir]
  
  by_year = list()
  
  if(full_years[i] < 2010){
    # Buscar archivos .dbf
    for(j in seq_along(folders_only)){
      folders_only_sub = list.files(folders_only[j], full.names = TRUE)
      
      for(a in seq_along(folders_only_sub)){
        file = folders_only_sub[a]
        
        if(grepl("\\.dbf$", file, ignore.case = TRUE)){
          name = basename(file)
          print(paste("Reading:", name))
          
          df <- tryCatch(read.dbf(file, as.is = TRUE),
    ...
    for(j in seq_along(folders_only)){
      folders_only_sub = list.files(folders_only[j], full.names = TRUE)
      folders_only_sub = folders_only_sub[file.info(folders_only_sub)$isdir]
      
      for(k in seq_along(folders_only_sub)){
        files_in_subsub = list.files(folders_only_sub[k], full.names = TRUE)
        
        for(a in seq_along(files_in_subsub)){
          file = files_in_subsub[a]
          ...
          if(grepl("\\.csv$", file, ignore.case = TRUE)){
            ...
            if(!is.null(df)){
              print(paste("✓ Archivo leído correctamente:", name))
              print(dim(df))
              by_year[[name]] <- df
            }
          }
  ...
  ENAHO_completo[[as.character(full_years[i])]] <- by_year
}
...
##################################################
#Gráfico con los todos o solo means de distritos mineros vs no mineros a
#través de los años evolución de los ingresos y de la pobreza (como se 
#presenta la pobreza en la ENAHO)
#Probablemente solo means pq presentar cada uno es complicado pq puede pasar de
#distrito minero a no minero con el tiempo, cómo hacer con eso?
##################################################
...
#Import data
ENAHO04_01_raw <- read.dbf ("./Data/ENAHO/2004/174-Modulo01 Características de la Vivienda y del Hogar/enaho01-2004-100.dbf")

view(ENAHO04_01_raw)
glimpse(ENAHO04_01_raw)

table(ENAHO04_01_raw$RESULT)

#Filtrar solo encuestas que completaron el cuestionario
ENAHO04_01 <- filter(ENAHO04_01_raw, RESULT==1)

#Existe variable NBI! Esto va a ser útil para estimar el monto de canon recibido
table(ENAHO04_01$NBI1)
table(ENAHO04_01$NBI2)
table(ENAHO04_01$NBI3)
table(ENAHO04_01$NBI4)
table(ENAHO04_01$NBI5)
...
###
ENAHO04_02_raw <- read.dbf ("./Data/ENAHO/2004/174-Modulo02 Características de los Miembros del Hogar/enaho01-2004-200.dbf")

view(ENAHO04_02_raw)
glimpse(ENAHO04_02_raw)
...
Ubigeo_distrito$inei = as.factor(Ubigeo_distrito$inei)



left_join(ENAHO04_05_1_raw, Ubigeo_distrito, by = c("UBIGEO" = "inei")) %>% 
  anti_join(., a, by = c("distrito" = "name")) %>% group_by(A_O) %>% summarize("mean"= mean(P524A1,  na.rm = TRUE))

anti_join(ENAHO04_05_1_raw, Ubigeo_distrito, by = c("UBIGEO" = "inei")) %>% names() #%>% nrow()


a = Transferencias_Municipales %>%
  mutate(name = str_remove(name, "^MUN\\. DIS\\. DE\\s+"))
